...
This project aims to develop a series of low-interaction honeypot decoy programs based on popular networking protocols (HTTP, FTP, and Telnet) that lure potential attackers using Tor Hidden Services (or Onions). The goal of the honeypots is to capture attacker interactions and encapsulate the information into logs which are processed using the Elastic Stack.

The project is comprised of the following:

- **HTTP Honeypot**: A Python-based Flask application that mimics a genuine Hypertext Transfer Protocol (HTTP) server, including user authentication and directory listings. This honeypot will record attempts to access the server, including specific pages visited and files downloaded.
- **FTP Honeypot:** A Python File Transfer Protocol (FTP) server utilising the `pyftpdlib` library that emulates a basic FTP service. Any login attempts will be logged by this honeypot, aiming to help profile attackers who target FTP services.
- **Telnet Honeypot**: A Python Telnet server that provides a realistic representation of an Apache Linux shell interface, complete with a limited set of commands. This honeypot intends to capture any login attempts to access the server.
- **ELK Stack**: A stack architecture comprising Elasticsearch, Logstash, and Kibana. Used to store the logs sent from the honeypots and render them into a format for visualisation purposes.

The attackers will have specialised enumeration methods to find the honeypot applications as they scan the Tor network. These decoy services can be used as a means to gather valuable intelligence about the techniques, tactics, and procedures (TTPs) used by malicious entities. Using this knowledge can ultimately lead to the improvement of a software's security implementations.

The development of these honeypot applications involved different technologies and programming paradigms. Most notably, Python (including libraries), Object-Oriented Programming (OOP), modular programming, and Docker. The reasoning behind these choices is to ensure that the honeypot applications are lightweight and reliable, whilst also incorporating security practices.
...
Each component can be called with an `import` statement when required, keeping each file simple.

The object-oriented paradigm further reinforces modularity and reusability, with classes encapsulating all similar data and functionality. This promotes code maintainability, readability, and extensibility, allowing for new features to be implemented without disrupting the original code.
...
In essence, the powerful combination of Python being developer-centric, cross-platform capabilities, and the application of both object-oriented and modular programming results in a honeypot solution that is easy to adapt, robust, and extensible to meet security requirements.

## 2.2 Containerisation and deployment

The decision to use Docker containers was mainly driven by the need for cross-platform compatibility, service isolation, and scalability of the honeypot applications. Packaging each service – the honeypot applications, Tor, and components of the ELK stack – with the relevant dependencies it requires, produces consistent yet reproducible deployments in different environments.
...
This container-based architecture approach offers multiple benefits. Since each Docker container is essentially a virtual machine running a lightweight fork of Linux. For every Docker container, excluding the ELK stack, a base image from the Alpine Linux distribution is used.

```dockerfile
FROM alpine:latest
```

This means each application can seamlessly run on a wide range of architectures being bundled with all the necessary packages. Not only is it beneficial in a production environment, but also when developing, the use of Docker image caching allows for the containers to be built quickly and efficiently each time a change is made [@rad2017]. It also circumvents the limitations users may have on their local machine, such as deprecated devices that don't support the latest packages – the only requirement being raw hardware power [@potdar2020].

In terms of security, each Docker container is isolated, so the attack surface is minimised. A vulnerability within one honeypot service would be contained, preventing cross-contamination with other containers [@bui2015]. Any exploits that attackers attempt to use will expose faults within that specific container.

# 3 Cyber security considerations

## 3.1 Integrating Tor hidden services

Including Tor hidden services was a vital design decision, as it provides a degree of anonymity for the honeypot applications and also helps to lure potential attackers. The accessibility of the honeypot applications, via Tor's `.onion` domains served as a major feature for this project, helping to further increase the availability of these decoy services to malicious entities scanning the Tor network.
